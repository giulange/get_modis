function list = nsidc_get_prodlist( platform, url )
% list = nsidc_get_satlist( url )
% 
% DESCRIPTION
%   This function is able to retrive the list of products available for
%   the given platform at the https://n5eil01u.ecs.nsidc.org portal, which
%   is used as default in case the input url is not provided.
% 
% INPUTS
...
% 
% OUTPUTS
%   list        Provide the list of all products available under the
%               provided platform.
...
[status,reply] = system(  wget_index( [url,'/',platform], DIR_TMP )  );
if status, error('%s',reply), end
html = greadtext( fullfile(DIR_TMP,platform) );
if size(html,2)>1
...
for ii = 1:numel(html)
    if Fdir(ii,1)==0
        ...
        iCount = iCount+1;
        LIST_PROD{iCount} = html{ii}(Fdir(ii,1):Fdir(ii,2));
    end
end
list = LIST_PROD;
